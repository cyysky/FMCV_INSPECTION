...
refresh_thread = Thread(target=image_thread, daemon=True)
refresh_thread.start()

frame = {"2":None}

def get_live(name=None):
    global frame
    if name is not None:
        return frame[name]
    return frame
...
def get_image(name=None):
    ...
    while previous == frame_int:
        pass
        
    #print("get_image time taken ",time.time() - start_time)
    if name is not None:
        return copy.deepcopy(frame[name])
